...
const signal_states_1 = require("./signal_states");
// Holds the current state of the intersection lights.
let curr_signal_state = signal_states_1.possible_signal_states.Left_Green;
...
let update_intersection = () => {
    ...
    console.log(`Left: {G: ${up_help(l.green)} Y: ${up_help(l.yellow)} R: ${up_help(l.red)} } | Right: {G: ${up_help(r.green)} Y: ${up_help(r.yellow)} R: ${up_help(r.red)} }`);
};
...
function L_Side() {
    ...
    curr_signal_state = signal_states_1.possible_signal_states.Left_Green;
    update_intersection();
    setTimeout(L_Yellow, time_green);
}
// Method to transition to the right side of the intersection.
function L_Yellow() {
    curr_signal_state = signal_states_1.possible_signal_states.Left_Yellow;
    update_intersection();
...
function R_Side() {
    ...
    curr_signal_state = signal_states_1.possible_signal_states.Right_Green;
    update_intersection();
    setTimeout(R_Yellow, time_green);
}
// Method to transition to the left side of the intersection.
function R_Yellow() {
    curr_signal_state = signal_states_1.possible_signal_states.Right_Yellow;
    update_intersection();
